...
#        weights <- wres
        ...
        ## PLOT SELECTED DAYS AND TRIALS ONLY
  ...
  write.csv(run_level_pc, "place_cell_fixed_run_level.csv", row.names = FALSE)
  
  trial_summary_pc <- do.call(rbind, rows_list)
  write.csv(trial_summary_pc, "place_cell_trial_mean_over_runs.csv", row.names = FALSE)
  saveRDS(PMs, "PMs_place_fixed.rds")
}
